...
- They are stored in the **macOS Keychain** (service `strap`), never written to disk in
  plaintext. Non-secret config lives in `~/.config/strap/` with `0600` permissions.
